...
#            setfeat 'disable cpus = smt ; turbo; monitorfreq ; monitormembw ; monitorcpupower ; monitorqpi'
            setfeat "no collectl; turbo on ;  cap cpu freq=${JFREQ}GHz"

            sbatch ../script/BM.run.sh -n $n -t 1 -l ${JFREQ}GHz -p $JPROC
done
done
done
}
