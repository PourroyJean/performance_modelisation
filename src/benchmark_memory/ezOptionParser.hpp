...
/* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const char *list, int _size) : s1(0), op(_op), quiet(0),
...
/* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const short *list, int _size) : s2(0), op(_op), quiet(0),
                                                                                               type(_type), size(_size),
                                                                                               insensitive(0) {
        id = ezOptionParserIDGenerator::instance().next();
        s2 = new short[size];
...
/* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const int *list, int _size) : s4(0), op(_op), quiet(0),
                                                                                             type(_type), size(_size),
                                                                                             insensitive(0) {
        id = ezOptionParserIDGenerator::instance().next();
        s4 = new int[size];
...
/* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const float *list, int _size) : f(0), op(_op), quiet(0),
                                                                                               type(_type), size(_size),
                                                                                               insensitive(0) {
        id = ezOptionParserIDGenerator::instance().next();
        f = new float[size];
...
/* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const double *list, int _size) : d(0), op(_op), quiet(0),
                                                                                                type(_type),
                                                                                                size(_size),
                                                                                                insensitive(0) {
        id = ezOptionParserIDGenerator::instance().next();
...
*/
    ezOptionValidator::ezOptionValidator(const char *_type, const char *_op, const char *_list, bool _insensitive) : t(
            0), quiet(0), type(0), size(0), insensitive(_insensitive) {
        id = ezOptionParserIDGenerator::instance().next();
...
/* ################################################################### */
    ...
    public:
        ...
        inline void configPrint(std::string &out);
...
/* ################################################################### */
    ...
    void ezOptionParser::configPrint(std::string &out) {
        char tmp[256];
        int i, j, k;
        ...
        for (i = 0; i < n; ++i) {
            ...
            sprintf(tmp, "%20s    ", g->flags.back()->c_str());
            out += tmp;

            if (g->isSet) {
                if (g->expectArgs) {
                    if (g->args.empty()) {
                        sprintf(tmp, "%-10s (default value)", g->defaults.c_str()); //DEFAULT
                        out += tmp;
                    } else {
                        for (k = 0; k < (long int) g->args.size(); ++k) {
                            for (j = 0; j < (long int) g->args[k]->size() - 1; ++j) {
...
//                sprintf(tmp, "Not set\n");
                sprintf(tmp, "%-10s (default value)", g->defaults.c_str());
                out += tmp;
            }
        }
        out += "\n";
