...
/*
	...
	enter the parms "parm_name=value"  in any order they all have default values
        ...
        MODE :
           ...
           MODE_READDML : special cases for stride 1
...
THEINT MEM_AFF = 1;                /* MEM ou le processeur doit tourner                              */
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//
//  ALGO for READDML // special case for 1,2,4
//
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

THETYPE
sum_readdml_unroll1(THETYPE *m, THEINT max_index, int step, int repeat, THEINT ops_per_scan) {
    THEINT steps;
    THETYPE sum = 0;
    int rep;
...
/*
void set_mpiaffinity()
{
	int ret;
	unsigned long long mask=1;
	mask <<= (((rank*MEM_AFF)+CPU_AFF)%64);
	ret = sched_setaffinity(0,  8, &mask);
}
*/
...
init_mat() {
    int lp;
    unsigned int m2;
    THEINT si = MAT_SIZE + 1;
    si * 1024;
    si * 1024;
    //set_mpiaffinity();
    //set_affinity(MEM_AFF);
...
work_part2(double *mat, int mode, int unroll, THEINT max_index, int step) {
    ...
    towrite = 0.0;
    if (mode == MODE_READDML) {
        if (unroll == UNROLL1) {
            return (sum_readdml_unroll1(mat, max_index, step, repeat, ops_per_scan));
        }/*
		if(unroll==UNROLL2){
			return(sum_readdml_unroll2(mat,max_index,step,repeat,ops_per_scan));
		}
		if(unroll==UNROLL4){
			return(sum_readdml_unroll4(mat,max_index,step,repeat,ops_per_scan));
		}
		if(unroll==UNROLL8){
			return(sum_readdml_unroll8(mat,max_index,step,repeat,ops_per_scan));
		}*/
...
init_uint64_parm(int argc, char **argv, THEINT *the_int, const char *name) {
    ...
    for (parm = 1; parm < argc; parm++) {
        ...
        if (where != NULL)
            if (where[strlen(name)] == '=') {
                *the_int = strtoul(where + strlen(name) + 1, 0, 10);
                if (VERBOSE != 0)
                    if (rank == 0)
                        printf("____ %s = %lld\n", name, *the_int);
            }
...
init_parms(int argc, char **argv) {
    ...
    init_int_with_str_parm(argc, argv, &MODE, "MODE", "READDML", MODE_READDML);
    if (MODE == MODE_READDML)moderw = MODER;
    ...
    assert((MODE == MODE_READ) || (MODE == MODE_READSPE) || (MODE == MODE_READIND) || (MODE == MODE_READDML) ||
